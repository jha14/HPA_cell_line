...
tcga_exp <- 
  fread("unzip -p ../../HPA_cell_line_data/HPA_TCGA_Cancer/rna_cancer_sample.tsv.zip")

tcga_exp$Gene %>% unique %>% length  # n = 19760
tcga_exp$Sample %>% unique %>% length  # n = 7932
tcga_exp$Cancer %>% unique %>% length  # n = 21

tcga_cancer_exp_avg <- tcga_exp %>% select(Gene, Cancer, FPKM) %>%
...
cl_mean_exp <- exp_landscape$combine %>% 
  left_join(hpa_tcga$cell_line_annotation, 
            by = c("sample" = "RRID")) %>% 
  select(ensg_id, ntpm, primary_disease) %>% 
...
# View(hpa_tcga$gsea_res$OV)
# View(hpa_tcga$gsea_res$HNSC)
...
for(i in names(hpa_tcga$tcga_disease_match)){
  hpa_tcga$combined_rank[[i]] <- 
    ...
    select(uniq_id, RRID, cell_line_name, correlation, correlation_rank = rank) %>% 
    left_join(hpa_tcga$gsea_res[[i]] %>% 
                select(uniq_id, gsea_rank = rank, NES, pval, padj, primary_disease, 
...
if(file.exists("results/Figure_4/cell_line_prioritization.xlsx"))
  unlink("results/Figure_4/cell_line_prioritization.xlsx")
for(i in names(hpa_tcga$combined_rank))
  write.xlsx(hpa_tcga$combined_rank[[i]], file = "results/Figure_4/cell_line_prioritization.xlsx", sheetName = i, row.names = F, append = T)
...
nes_range <- c()
for(i in names(hpa_tcga$tcga_disease_match)){
  correlation_range <- c(correlation_range, hpa_tcga$combined_rank[[i]] %>% filter(overall_rank <= 10) %>% select(correlation) %>% pull)
  nes_range <- c(nes_range, hpa_tcga$combined_rank[[i]] %>% filter(overall_rank <= 10) %>% select(NES) %>% pull)
}
...
hpa_tcga$tcga_gene_short <- hpa_tcga$tcga_cancer_gene_classification %>% select(spec_category, enriched_tissues) %>% 
...
ggsave("results/Figure_S4/TCGA_cohort_bar.pdf", height = 4, width = 10, dpi = 600, units = "in")



# TCGA cohort gene signature in cell line -----------------------------------------

lihc_signature = hpa_tcga$tcga_cancer_gene_classification %>% select(gene, spec_category, enriched_tissues) %>% 
  filter(spec_category %in% c("group enriched", "tissue enhanced", "tissue enriched"))
...
lihc_signature_dot_long <- lihc_signature_dot %>% rownames_to_column("ensg_id") %>% 
  ...
  left_join(hpa_tcga$combined_rank$LIHC %>% select(RRID, cell_line_name), by = c("lihc_cell_line" = "RRID")) %>% 
...
# lihc_signature_dot["ENSG00000163631",]
# lihc_signature_dot_long %>% filter(ensg_id == "ENSG00000143627")
...
lihc_signature_bar <- 
  lihc_signature_bar %>% 
  left_join(hpa_tcga$combined_rank$LIHC %>% select(RRID, cell_line_name), by = c("cell_line" = "RRID")) %>% 
  mutate(cell_line_name = factor(cell_line_name, levels = hpa_tcga$combined_rank$LIHC$cell_line_name))
...
liver_cell_line_exp <- 
  hpa_tcga$cell_line_exp_aligned[,hpa_tcga$cell_line_annotation %>% filter(primary_disease == "Liver Cancer") %>% select(RRID) %>% pull]
...
lihc_corr_plot <- 
  ...
  left_join(lihc_signature %>% select(Gene = gene, spec_category))
...
ggplot(lihc_corr_plot, aes(x = LIHC, y = CVCL_0077)) + 
  ...
  theme(panel.background = element_rect(colour = "black",size = 1),aspect.ratio=1,
        ...
        axis.text.x = element_text(size = 14, colour = "black"), axis.title.x = element_text(size = 15, colour = "black"), 
        axis.title.y = element_text(size = 15, colour = "black"), axis.text.y = element_text(size = 14, colour = "black"),
        legend.position = "right", legend.title = element_blank(), legend.text = element_text(size = 15, colour = "black")) + 
  xlab(expression(paste("Log"[2], "(FPKM+1) LIHC"))) + ylab(expression(paste("Log"[2], "(nTPM+1) SNU-398")))
ggsave(file = "results/Figure_4/LIHC_cell_line_legend.pdf", dpi = 600, width = 4, height = 4)
...
cl_tcga_aligned_alluvial$alluvial <- exp_landscape$hpa_summary %>%
  select(Ensembl, cancer_specificity = 'RNA cancer specificity') %>%
...
cl_tcga_aligned_alluvial$alluvial_long <- 
  to_lodes_form(cl_tcga_aligned_alluvial$alluvial %>% select(Ensembl, cl_disease_tcga, cancer_specificity), key = "category", axes = c(2,3))
...
ggsave("results/Figure_2/flow_specificity_cl_tcga_aligned.pdf", width = 6, height = 6, dpi = 600, bg = "white")
...
immune_go_genes$tcell_cl %>% 
  ...
  theme(panel.border = element_blank(), 
        ...
        legend.title = element_text(size = 11, colour = "black"),
        legend.text = element_text(size = 10, colour = "black"))
ggsave("results/Figure_2/T_cell_genes_cl.pdf", height = 5, width = 7, dpi = 600, units = "in")
...
immune_go_genes$tcell_tcga %>% 
  ...
  theme(panel.border = element_blank(), 
        ...
        legend.title = element_text(size = 11, colour = "black"),
        legend.text = element_text(size = 10, colour = "black"))
ggsave("results/Figure_2/T_cell_genes_tcga.pdf", height = 5, width = 7, dpi = 600, units = "in")
...
# MCF-7, A-549 location

# alpha_dot <- ifelse(hpa_tcga$combined_rank[["LUSC"]]$cell_line_name == "A-549", "red", "blue")
# p <- hpa_tcga$combined_rank[["LUSC"]] %>% 
#   ggplot(aes(x = NES, y = correlation)) +
#   geom_point(aes(color = alpha_dot), size = 3, shape = 16) +
#   theme_bw() + 
#   scale_x_continuous(breaks = seq(0.6, 2.4, 0.6)) +
#   scale_y_continuous(breaks = seq(0.64, 0.8, 0.04)) +
#   scale_color_manual(values = c("red" = "#F8766D", "blue" = "grey60")) +
...
tau$cell_line_disease <-
  exp_landscape$combine_disease %>%
  ungroup %>%
  rename(gene = ensg_id, exp = ntpm) %>%
  mutate(exp = log10(exp+1)) %>% 
  calculate_tau

tau$sc_type <-
  read_tsv("data/rna_single_cell_type.tsv") %>%
  select(gene = Gene, cell_type = 'Cell type', nTPM) %>%
...
tau$plot <-
  ...
  left_join(exp_landscape$gene_disease %>% select(gene, cell_line_spec = spec_category)) %>%
  left_join(exp_landscape$hpa_summary %>% select(gene = Ensembl,
...
ggsave(file = "results/Figure_2/CL_vs_Cancer_tau.pdf", dpi = 600, width = 5, height = 5)

# nortest::ad.test(tau$plot$cell_line_disease_tau)
# nortest::ad.test(tau$plot$cancer_tau)
# nortest::ad.test(tau$plot$sc_type_tau)
cor(tau$plot$cell_line_disease_tau, tau$plot$cancer_tau, method = "spearman")
...
ggsave(file = "results/Figure_2/CL_vs_SCtype_tau.pdf", dpi = 600, width = 5, height = 5)
cor(tau$plot$cell_line_disease_tau, tau$plot$sc_type_tau, method = "spearman")
...
ggplot(tau$plot, aes(x = cell_line_disease_tau, y = cell_line_spec)) +
  ...
  theme(panel.border = element_blank(), aspect.ratio = 0.4,
        ...
        axis.line = element_line(), axis.ticks.length = unit(.1, "cm"),
        legend.position = "none")
ggsave(file = "results/Figure_2/tau_cell_line_tau.pdf", dpi = 600, width = 4, height = 2.5)
...
ggplot(tau$plot, aes(x = cancer_tau, y = cancer_spec)) +
  ...
  theme(panel.border = element_blank(), aspect.ratio = 0.4,
        ...
        axis.line = element_line(), axis.ticks.length = unit(.1, "cm"),
        legend.position = "none")
ggsave(file = "results/Figure_2/tau_cancer_tau.pdf", dpi = 600, width = 4, height = 2.5)
...
ggplot(tau$plot, aes(x = sc_type_tau, y = sc_spec)) +
  ...
  theme(panel.border = element_blank(), aspect.ratio = 0.4,
        ...
        axis.line = element_line(), axis.ticks.length = unit(.1, "cm"),
        legend.position = "none")
ggsave(file = "results/Figure_2/tau_sc_type_tau.pdf", dpi = 600, width = 4, height = 2.5)
...
# CESC: Cervical Squamous Cell Carcinoma and Endocervical Adenocarcinoma, Cervix uteri
# COAD: Colon Adenocarcinoma, Colon / Rectosigmoid junction
...
# LIHC: Liver Hepatocellular Carcinoma, Liver and intrahepatic bile ducts
# LUAD: Lung Adenocarcinoma, Bronchus and lung
# LUSC: Lung Squamous Cell Carcinoma, Bronchus and lung
