...
ggsave(file = "results/Figure_S5/UMAP_CytoSig.pdf", width = 5, height = 5.3, units = "in", dpi = 600)
...
column_ha <- HeatmapAnnotation(Disease = cytosig_analysis$annotation_heatmap$primary_disease,
                               col = list(Disease = disease_colors_combine1),
                               annotation_legend_param = list(Disease = list(ncol = 7)))

pdf("results/Figure_5/cytosig_heatmap.pdf", width = 14, height = 9)
draw(Heatmap(cytosig_analysis$cytosig_results[,cytosig_analysis$annotation_heatmap$cell_line_name], 
...
pdf("results/Figure_5/cytosig_heatmap_2.pdf", width = 12, height = 10)
...
pdf("results/Figure_S5/cytosig_cor.pdf", width = 11, height = 11)
...
pdf("results/Figure_S5/progeny_cytosig_cor.pdf", width = 6, height = 10)
Heatmap(t(progeny_cytosig_cor), show_column_names = T, show_row_names = T,
        width = nrow(progeny_cytosig_cor)*unit(5, "mm"),
        height = ncol(progeny_cytosig_cor)*unit(5, "mm"), 
        col = colorRamp2(c(-1, 0, 1), c("#2166AC", "white", "#B2182B")),
        heatmap_legend_param = list(title = "Spearman's ρ", at = c(-1, -0.5, 0, 0.5, 1), direction = "horizontal"))
dev.off()

pdf("results/Figure_S5/progeny_cytosig_cor2.pdf", width = 15, height = 6)
...
plot_cytosig_cytokine <- function(plot_cell_line){
  plot_cytosig = cytosig_analysis$cytosig_results %>% 
    as.data.frame %>% 
    ...
    mutate(pval = cytosig_analysis$cytosig_results_p %>% select(plot_cell_line) %>% pull) %>% 
    mutate(sig = ifelse(pval < 0.05, "sig", "nonsig") %>% factor(levels = c("sig", "nonsig"))) %>% 
    mutate(y = ifelse(score > 0, -max(abs(score))/20, max(abs(score))/20), 
           x = 1:43, hjust = ifelse(score > 0, 1, 0)) %>% 
  ...
  yrange <- plot_cytosig$score %>% abs %>% max %>% ceiling
  
  ggdotchart(plot_cytosig, x = "pathway", y = "score", sorting = "none", dot.size = plot_cytosig$size,
             add = "segments", xlab="CytoSig cytokines", title = plot_cell_line, shape = 21) +
    geom_point(aes(fill = direction, color = sig), shape = 21, size = plot_cytosig$size) + 
    geom_text(aes(label = pathway, x = x, y = y, hjust = hjust), vjust = 0.5, size = 4, angle = 90) +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), 
          axis.line.x = element_blank(), legend.position = c(0.93, 1.05),
  ...
  ggsave(paste0("results/CytoSig/", str_replace_all(plot_cell_line, "/", "_"), ".pdf"), width = 10, height = 3.5)
}

for(i in colnames(cytosig_analysis$cytosig_results))
  plot_cytosig_cytokine(i)
