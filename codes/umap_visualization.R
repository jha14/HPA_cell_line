...
umap_cell_lines_intersect %>% 
  ...
  geom_point(aes(shape = data_source), size = umap_cell_lines_intersect$size, 
             alpha = ifelse(umap_cell_lines_intersect$size == 1, 0.5, 1)) +
...
pdf(file = "results/Figure_S3/dendrogram_bar1.pdf", height = 10, width = 10)
...
pdf(file = "results/Figure_S3/dendrogram_bar2.pdf", height = 10, width = 10)
circos.initialize(df$sectors, x = df$x)
circos.track(ylim = c(0, 1), track.height = 0.05, 
             track.margin = c(0, 0),
             bg.border = NA,
